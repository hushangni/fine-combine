...
fineApp.grid = {
    ...
    inBounds(position) {
        if (
        position.x >= 0 &&
        position.x < fineApp.gridSize &&
        position.y >= 0 &&
        position.y < fineApp.gridSize
        ) {
        return true;
        } else {
        return false;
...
fineApp.updateBoard = () => {
    window.requestAnimationFrame(function() {
        ...
        $("#tweet")
            .attr({
            href:
                'https://twitter.com/intent/tweet?text="I just got Juicy in ' +
                fineApp.minutes +
                "m " +
                fineApp.seconds +
                "s" +
                " with @shangniwho " +
                "'s" +
                ' 🍑 Juicy Jostle! Think you can get Juicy faster? 💪 https://bit.ly/2nQPjCm"',
            target: "_blank"
            })
            .addClass("")
        ...
        dbRef.on("value", snapshot => {
            const scoreArray = Object.entries(snapshot.val()).map(item => {
            return {
                name: item[1].name,
                score: item[1].score,
                time: item[1].time
...
fineApp.init = () => {
    ...
    $(".fa-save").on("click", function(e) {
        ...
        $(".high-scores").append(
            `<li class="score"><span class="player-time">${player.time}:</span> ${
            player.name
            }</li>`
        );
        });
...
$(function() {
    ...
    // loser cheat code
    let pressed3 = [];
    const secretCode3 = "loser";
    window.addEventListener("keyup", e => {
        pressed3.push(e.key);
        pressed3.splice(-secretCode3.length - 1, pressed3.length - secretCode3);
    ...
    window.addEventListener("keyup", e => {
        pressed1.push(e.key);
        pressed1.splice(-secretCode1.length - 1, pressed1.length - secretCode1);
    ...
    // back to fruit images cheat code
    let pressed2 = [];
    const secretCode2 = "fruits";
    window.addEventListener("keyup", e => {
        pressed2.push(e.key);
        pressed2.splice(-secretCode2.length - 1, pressed2.length - secretCode2);
